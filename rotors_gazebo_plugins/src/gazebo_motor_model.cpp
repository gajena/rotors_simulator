  ...
  switch (motor_type_) {
    case (MotorType::kPosition): {
      double err = NormalizeAngle(joint_->Position(0)) -
                   NormalizeAngle(ref_motor_input_);
    ...
    default:  // MotorType::kVelocity
    {
      ...
      ignition::math::Vector3d relative_wind_velocity_W =
          body_velocity_W - wind_speed_W_;
      ...
      ref_motor_rot_vel = rotor_velocity_filter_->updateFilter(ref_motor_input_,
                                                               sampling_time_);

      // Make sure max force is set, as it may be reset to 0 by a world reset
      // any time. (This cannot be done during Reset() because the change will
      // be undone by the Joint's reset function afterwards.)
